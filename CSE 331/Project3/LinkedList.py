...
class LinkedList:
    ...
    def insertion_sort(self):
        """
        This function will Use insertion sort to sort the current instance of the linked list.
        :param : The current linked list
        :return: None
        """
        if self.is_empty() or self.length() == 1:
            return self
        sorted = LinkedList()
        sorted.push_front(self.head.value)
        ...
        while next is not None:
            ...
            while head is not None:
                if next.value >= head.value:
                    head = head.next_node
                    if head is None:
                        sorted.push_back(next.value)
                        break
                    continue
                if next.value < head.value and head == sorted.head:
                    sorted.push_front(next.value)
                    break
                if next.value < head.value:
                    if head == sorted.tail:
                        temp2 = Node(head.value, head.next_node)
                        head.value = next.value
                        head.next_node = temp2
                        sorted.tail = temp2
                        break
                    temp2 = Node(head.value, head.next_node)
                    head.value = next.value
